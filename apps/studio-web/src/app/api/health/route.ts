...
export async function GET() {
  ...
  let schema: SchemaState = { control_plane_ready: false, workforce_ready: false, runtime_ready: false, audit_ready: false };
    ...
    try {
      const [row] = await sql<SchemaState[]>`
        SELECT
          ...
          to_regclass('public.runtime_test_runs') IS NOT NULL
            AND to_regclass('public.provider_health') IS NOT NULL
            AND to_regclass('public.runtime_events') IS NOT NULL
            AND to_regclass('public.runtime_usage') IS NOT NULL AS runtime_ready,
  ...
  const operational = database === "reachable" && schema.control_plane_ready && schema.workforce_ready && schema.runtime_ready && schema.audit_ready;
  return NextResponse.json({
    ...
    capabilities: {
      ...
      studio: true,
      presenterDraft: true,
      testCentre: process.env.ENABLE_TEST_CENTRE !== "false",
      sandboxTaskRunner: process.env.ENABLE_SANDBOX_TASK_RUNNER !== "false",
      modelExecution: process.env.ENABLE_WORKFORCE_MODEL_EXECUTION === "true" && Boolean(process.env.OPENAI_API_KEY),
      productionRuntime: process.env.ENABLE_PRODUCTION_RUNTIME === "true",
      voice: "adapter-ready",
      realtime: process.env.ENABLE_OPENAI_REALTIME === "true" ? "configured" : "disabled",
      billing: "disabled",
      email: "disabled",
      gpu: process.env.AVATAR_WORKER_URL || process.env.GPU_WORKER_URL ? "configured" : "disabled",
    },
