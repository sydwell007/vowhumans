...
function ReplicaCaptureWorkflow({ detail, storageConfigured, onRefresh }: { detail: ReplicaDetail; storageConfigured: boolean; onRefresh: () => Promise<void> }) {
  ...
  return (
    <div>
      ...
      {step === 10 && <section className="replica-gate"><p className="eyebrow">Private processing</p><h3>{processingFailed ? latestJob?.status === "failed" ? "Processing failed" : "Capture did not pass automated quality checks" : latestJob ? `Processing ${latestJob.status.replaceAll("_", " ")}` : "Not submitted"}</h3><p>{processingFailed ? <CircleAlert size={17} /> : latestJob?.status === "completed" ? <CircleCheck size={17} /> : <Clock3 size={17} />}{processingFailed ? latestJob?.safe_error_code ? `The processor reported ${latestJob.safe_error_code.replaceAll("_", " ").toLowerCase()}. Return to the affected capture step and record or upload that clip again.` : "Record or upload a compliant capture, then run processing again. Failed measurements are retained as audit evidence." : latestJob ? `Progress ${latestJob.progress}%. This page refreshes automatically while the dedicated processor validates footage and writes a private motion manifest.` : "Complete the capture quality gate first."}</p>{automaticChecks.length > 0 && <ul>{automaticChecks.map((check) => <li key={check.check_code}><StatusPill tone={toneForStatus(check.status)}>{check.status}</StatusPill><span>{check.check_code.replaceAll("_", " ")}{check.measured_value != null ? `: ${check.measured_value}${check.unit ? ` ${check.unit}` : ""}` : ""}{check.threshold_value != null ? ` (required ${check.threshold_value}${check.unit ? ` ${check.unit}` : ""})` : ""}</span></li>)}</ul>}<button className="secondary-button" type="button" onClick={onRefresh}><RefreshCw size={17} />Refresh processing</button></section>}
