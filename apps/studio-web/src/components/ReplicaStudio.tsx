...
function ReplicaCaptureWorkflow({ detail, storageConfigured, onRefresh }: { detail: ReplicaDetail; storageConfigured: boolean; onRefresh: () => Promise<void> }) {
  ...
  return (
    <div>
      ...
      {step === 11 && <QualityEvidenceReview profileId={detail.profile.id} humanSlug={detail.profile.human_slug} checks={detail.quality_checks} processingReady={detail.profile.status === "quality_review" || detail.profile.status === "approved"} onRefresh={onRefresh} />}
...
function QualityEvidenceReview({ profileId, humanSlug, checks, processingReady, onRefresh }: { profileId: string; humanSlug: string | null; checks: ReplicaDetail["quality_checks"]; processingReady: boolean; onRefresh: () => Promise<void> }) {
  const [visualNotes, setVisualNotes] = useState("");
  const [busy, setBusy] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [previewLatency, setPreviewLatency] = useState<number | null>(null);
  const [liveRoom, setLiveRoom] = useState<{ url: string; token: string; room: string; sessionId: string } | null>(null);
  ...
  const average = runs.length ? Math.round(runs.reduce((sum, value) => sum + value, 0) / runs.length) : null;
  ...
  async function generatePreview() {
    ...
    try {
      const response = await fetch(`/api/v1/replicas/${profileId}/preview`, { method: "POST", headers: { "content-type": "application/json" }, body: "{}" });
      if (!response.ok) {
        const payload = await response.json().catch(() => null) as { message?: string } | null;
        throw new Error(payload?.message || "The staged replica preview could not be generated.");
      }
      const nextUrl = URL.createObjectURL(await response.blob());
      setPreviewUrl((current) => { if (current) URL.revokeObjectURL(current); return nextUrl; });
      setPreviewLatency(Number(response.headers.get("x-vowhumans-preview-latency-ms")) || null);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "The staged replica preview could not be generated."); }
  ...
  async function startLatencyTest() {
    if (!humanSlug) { setError("Link this replica to a Digital Human before starting the LiveKit test."); return; }
    setBusy("livekit-start"); setError(null); setRuns([]); setMuted(false);
    try {
      const session = await api<{ session_id: string }>("/api/v1/live-sessions", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ digital_human_id: humanSlug, staged_replica_profile_id: profileId }) });
      const token = await api<{ url: string; token: string; room: string }>(`/api/v1/live-sessions/${session.session_id}/token`, { method: "POST" });
      setLiveRoom({ ...token, sessionId: session.session_id });
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not start the staged LiveKit test."); }
    finally { setBusy(null); }
  }

  async function endLatencyTest() {
    const sessionId = liveRoom?.sessionId;
    setLiveRoom(null); setLiveStatus(null); setMuted(true);
    speechStartedRef.current = false; speechEndedAtRef.current = null; responseStartedRef.current = false;
    if (sessionId) await fetch(`/api/v1/live-sessions/${sessionId}/end`, { method: "POST" }).catch(() => undefined);
  }

  function localSpeakingChanged(speaking: boolean) {
    if (speaking) { speechStartedRef.current = true; speechEndedAtRef.current = null; responseStartedRef.current = false; }
    else if (speechStartedRef.current) { speechStartedRef.current = false; speechEndedAtRef.current = performance.now(); }
  }

  function agentSpeakingChanged(speaking: boolean) {
    if (speaking && speechEndedAtRef.current !== null) responseStartedRef.current = true;
  }

  function avatarFrame(timestamp: number) {
    if (!responseStartedRef.current || speechEndedAtRef.current === null) return;
    const measured = Math.max(1, Math.round(timestamp - speechEndedAtRef.current));
    responseStartedRef.current = false; speechEndedAtRef.current = null;
  ...
  function networkProfile() {
    ...
    return connection ? `${connection.effectiveType ?? "unknown"}, ${connection.downlink ?? "?"} Mbps downlink, ${connection.rtt ?? "?"} ms RTT` : `${navigator.userAgent}; browser network metrics unavailable`;
  }

  return <section className="replica-quality-review"><div><p className="eyebrow">Human + runtime evidence</p><h3>Review source preservation and LiveKit delivery</h3><p>Generate the staged output here, compare it with the private source, then run the separate real-room latency test.</p></div>{error && <div className="review-warning"><CircleAlert size={17} />{error}</div>}<div className="replica-preview-actions"><button className="primary-button" type="button" disabled={!processingReady || busy !== null} onClick={generatePreview}>{busy === "preview" ? <RefreshCw className="spin" size={17} /> : <Sparkles size={17} />}{busy === "preview" ? "Generating on GPU…" : previewUrl ? "Regenerate Photoreal Replica preview" : "Generate Photoreal Replica preview"}</button><small>This creates a review-only mouth-retargeted video. It does not approve or publish the replica.</small></div><div className="replica-preview-comparison"><article><StatusPill tone="muted">Source capture</StatusPill><h3>Original preserved motion</h3><video controls playsInline preload="metadata" src={`/api/v1/replicas/${profileId}/source-preview`} /></article><article><StatusPill tone={previewUrl ? "good" : "muted"}>{previewUrl ? "Generated" : "Not generated"}</StatusPill><h3>Generated Photoreal Replica</h3>{previewUrl ? <video controls playsInline src={previewUrl} /> : <div className="replica-preview-placeholder"><Video size={28} /><span>Use “Generate Photoreal Replica preview” above.</span></div>}{previewLatency && <small>Staged generation took {previewLatency} ms. This is not the LiveKit room latency.</small>}</article></div><div className="replica-evidence-grid"><article><StatusPill tone={toneForStatus(latest("lip_sync_visual_review"))}>{latest("lip_sync_visual_review").replaceAll("_", " ")}</StatusPill><h3>Visual preservation</h3><p>Compare the two videos above. Confirm mouth sync while blink, gaze, breath, shoulders and hand gestures remain unchanged.</p><textarea value={visualNotes} onChange={(event) => setVisualNotes(event.target.value)} placeholder="Reviewer, source/output clips, observed sync and motion integrity…" /><div className="editor-actions"><button className="primary-button" disabled={!previewUrl || visualNotes.trim().length < 10 || busy !== null} type="button" onClick={() => record("lip_sync_visual_review", "passed", visualNotes)}>Pass with evidence</button><button className="secondary-button" disabled={!previewUrl || visualNotes.trim().length < 10 || busy !== null} type="button" onClick={() => record("lip_sync_visual_review", "failed", visualNotes)}>Fail</button></div></article><article className="replica-livekit-test"><StatusPill tone={toneForStatus(latest("livekit_latency"))}>{latest("livekit_latency").replaceAll("_", " ")}</StatusPill><h3>LiveKit latency</h3><p>Start the staged room, speak a short sentence, then stop. Each genuine result measures from the end of your speech to the first decoded responsive replica frame. Complete three runs.</p>{liveRoom ? <><div className="replica-livekit-stage"><LiveVoiceRoom url={liveRoom.url} token={liveRoom.token} muted={muted} onStatusChange={setLiveStatus} onLocalSpeakingChange={localSpeakingChanged} onSpeakingChange={agentSpeakingChanged} onAvatarVideoFrame={avatarFrame} /><span>{liveStatus === "connected" ? muted ? "Microphone muted" : "Connected — speak, then pause" : liveStatus ?? "Connecting…"}</span></div><div className="editor-actions"><button className="secondary-button" type="button" onClick={() => setMuted((value) => !value)}>{muted ? <MicOff size={16} /> : <Mic size={16} />}{muted ? "Unmute" : "Mute"}</button><button className="secondary-button" type="button" onClick={endLatencyTest}><PhoneOff size={16} />End test</button></div></> : <button className="primary-button" type="button" disabled={!previewUrl || latest("lip_sync_visual_review") !== "passed" || busy !== null || !humanSlug} onClick={startLatencyTest}>{busy === "livekit-start" ? <RefreshCw className="spin" size={16} /> : <Mic size={16} />}Start LiveKit latency test</button>}<div className="replica-latency-runs"><strong>{runs.length}/3 genuine runs</strong>{runs.map((value, index) => <span key={`${index}-${value}`}>Run {index + 1}: {value} ms</span>)}{average !== null && <span>Average: {average} ms {average <= 1500 ? "— passes threshold" : "— exceeds 1,500 ms"}</span>}</div><div className="editor-actions"><button className="primary-button" disabled={runs.length < 3 || average === null || !liveRoom || busy !== null} type="button" onClick={() => record("livekit_latency", average! <= 1500 ? "passed" : "failed", `Room ${liveRoom!.room}; Docker image syd001/vowhumans-avatar-worker:v15; network ${networkProfile()}; runs ${runs.map((value, index) => `${index + 1}=${value}ms`).join(", ")}; average=${average}ms.`, average!)}>Record measurement</button></div></article></div></section>;
}
