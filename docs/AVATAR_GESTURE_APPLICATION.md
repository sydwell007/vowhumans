...
- **Next.js / Vercel** (Studio UI, the internal gesture-resolution API, Presenter Studio's render-next-scene wiring): deploys automatically via `vercel --prod`, as usual.
- **`avatar-worker`** (the actual MuseTalk + blink/sway rendering code — a GPU pod on RunPod/Modal, per `docs/DEPLOYMENT.md`) and **`avatar-participant`** (the LiveKit worker that fetches the gesture profile and forwards it — a Railway service, per `docs/LIVE_VOICE_DEPLOYMENT.md`, which says outright: *"Nothing here is automated"*): both are **operator-run manual deployments**, entirely separate from the Vercel deploy, and this session has no credentials for either platform.

A `vercel --prod` deploy — including the one already run for the *first* gesture-application pass — never touches either of those services. If Thandi's rendered call shows lip-sync but no head tilt/nod/breathing-sway/blink at all, the near-certain explanation is that `avatar-worker` is still running the pre-gesture image (it wouldn't even have `gesture_sway.py` yet, let alone the new `blink_synth.py`) and/or `avatar-participant` predates the `_fetch_gesture` call added in the first pass. Both need a fresh operator-triggered image rebuild + redeploy on their respective platforms before any of this — including the original head-tilt/nod/sway work — becomes visible in a real call. This is not a new gap this pass introduced; it's the same gap flagged (and left unresolved, for lack of credentials) at the end of the previous pass.
