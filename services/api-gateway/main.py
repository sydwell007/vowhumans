...
def create_livekit_token(room: str, participant: str, organisation_id: uuid.UUID, session_id: uuid.UUID, human_slug: str | None, persona_version_id: uuid.UUID | None, requested_language: str | None = None, staged_replica_profile_id: uuid.UUID | None = None) -> str:
...
def livekit_token(body: LiveKitTokenRequest, auth: Auth):
    ...
    token = create_livekit_token(room, body.participant_identity, auth.organisation_id, body.session_id, body.human_slug, body.persona_version_id, body.requested_language, body.staged_replica_profile_id)
