...
class LiveKitTokenRequest(BaseModel):
    ...
    staged_replica_profile_id: uuid.UUID | None = None
    renderer_tier: Literal["live_voice", "rigged_3d"] = "live_voice"

TOKEN_TTL = datetime.timedelta(seconds=600)
VOICE_AGENT_NAME = "vowhumans-voice"
AVATAR_AGENT_NAME = "vowhumans-avatar"

def create_livekit_token(room: str, participant: str, organisation_id: uuid.UUID, session_id: uuid.UUID, human_slug: str | None, persona_version_id: uuid.UUID | None, requested_language: str | None = None, staged_replica_profile_id: uuid.UUID | None = None, renderer_tier: str = "live_voice") -> str:
    ...
    if human_slug and renderer_tier != "rigged_3d" and os.getenv("ENABLE_AVATAR_PARTICIPANT", "false").lower() == "true":
        avatar_metadata = json.dumps({"organisation_id": str(organisation_id), "human_slug": human_slug, "staged_replica_profile_id": str(staged_replica_profile_id) if staged_replica_profile_id else None})
...
def livekit_token(body: LiveKitTokenRequest, auth: Auth):
    ...
    token = create_livekit_token(room, body.participant_identity, auth.organisation_id, body.session_id, body.human_slug, body.persona_version_id, body.requested_language, body.staged_replica_profile_id, body.renderer_tier)
